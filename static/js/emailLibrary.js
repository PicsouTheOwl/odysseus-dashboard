...
export function openEmailLibrary(opts = {}) {
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete Boubane reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show Boubane reminder emails">
  ...
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all Boubane reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
...
async function _toggleCardPreview(card, em) {
  ...
  try {
      ...
      <div class="email-reader-header">
        ...
        <div class="email-reader-actions">
          ...
          <div class="email-reader-actions-row email-reader-actions-row-secondary">
            ...
            <button class="memory-toolbar-btn reader-icon-btn owl-agent-btn" data-act="owl-agent" title="Discuter avec OWL Agent"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/><path d="M12 2v2"/><path d="M12 20v2"/><path d="M2 12h2"/><path d="M20 12h2"/></svg><span class="reader-btn-label">OWL</span></button>
    ...
    reader.querySelector('[data-act="owl-agent"]')?.addEventListener('click', (ev) => {
      ev.stopPropagation();
      // Inject email context into the main chat input
      const subject = data.subject || '(pas de sujet)';
      const from = data.from_name || data.from_address || '';
      const body = (data.body || '').slice(0, 500);
      const ctxMsg = `📧 Mail: "${subject}" de ${from}\n\n${body}\n\n---\nPose-moi une question ou demande-moi de répondre/résumer.`;
      const msgInput = document.getElementById('message');
      if (msgInput) {
        msgInput.value = ctxMsg;
        msgInput.focus();
        // Dispatch input event for auto-resize
