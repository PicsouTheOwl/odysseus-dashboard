...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Boubane Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Boubane kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Boubane Chat';
    }
