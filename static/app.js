// ============================================
// Boubane UI — Main Application Orchestrator
...
import { openOwlAgent, closeOwlPanel } from './js/hermesAgent.js';
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Boubane Chat';
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Boubane...');
      }
...
// ============================================
// OWL AGENT — Hermes integration
// ============================================
function initOwlAgent(apiBase) {
  // Check bridge health & update status indicator
  async function checkOwlStatus() {
    const statusEl = document.getElementById('owl-status');
    if (!statusEl) return;
    const dot = statusEl.querySelector('.owl-status-dot');
    try {
      const r = await fetch('http://127.0.0.1:8643/health', {
        signal: AbortSignal.timeout(3000)
      });
      if (r.ok) {
        dot.classList.remove('offline');
        statusEl.title = 'OWL Agent — Connecté';
      } else {
        dot.classList.add('offline');
        statusEl.title = 'OWL Agent — Déconnecté';
      }
    } catch {
      dot.classList.add('offline');
      statusEl.title = 'OWL Agent — Hors ligne';
    }
  }

  // Initial check + periodic refresh
  checkOwlStatus();
  setInterval(checkOwlStatus, 30000);

  // Click on status → open chat with OWL context
  const statusEl = document.getElementById('owl-status');
  if (statusEl) {
    statusEl.addEventListener('click', () => {
      // Switch to chat section if not already there
      const chatBtn = document.getElementById('rail-chats');
  ...
  // Make OWL the default chat backend
  window.__odysseusUseOwl = true;

  console.log('🦉 OWL Agent initialized — bridge at 8643');
}

// ============================================
// INITIALIZATION ON PAGE LOAD
// ============================================
function startBoubaneApp() {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startBoubaneApp, { once: true });
} else {
  startBoubaneApp();
}
