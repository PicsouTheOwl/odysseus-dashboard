...
def setup_chat_routes(
    ...
    # ------------------------------------------------------------------ #
    # POST /api/owl/stream — Chat via OWL (Hermes Agent) en streaming SSE
    # ------------------------------------------------------------------ #
    @router.post("/api/owl/stream")
    async def owl_stream(request: Request) -> StreamingResponse:
        """Proxy de chat vers le bridge Hermes OWL avec streaming SSE."""
        import httpx
        ...
        preset_id = form_data.get("preset_id") or (body or {}).get("preset_id", "")
        ...
        async def _stream_owl():
            """Proxy SSE vers le bridge Hermes OWL."""
            try:
                bridge_url = "http://127.0.0.1:8643/chat/stream"

                payload = {
                    "message": str(message),
                    "session_id": str(session) if session else "default",
                    "context": f"Dashboard session: {session}, preset: {preset_id}, owner: {owner}",
                    "stream": True
                }

                async with httpx.AsyncClient(timeout=120) as client:
                    async with client.stream("POST", bridge_url, json=payload) as resp:
                        if resp.status_code != 200:
                            error_msg = f"Bridge error: {resp.status_code}"
                            err_payload = json.dumps({"error": error_msg, "status": resp.status_code})
                            yield f"event: error\ndata: {err_payload}\n\n"
                            return
                ...
                err_payload = json.dumps({"error": "OWL timeout", "status": 504})
                yield f"event: error\ndata: {err_payload}\n\n"
            except Exception as e:
                logger.error(f"OWL stream error: {e}")
                err_payload = json.dumps({"error": f"OWL error: {str(e)}", "status": 500})
                yield f"event: error\ndata: {err_payload}\n\n"

        return StreamingResponse(_stream_owl(), media_type="text/event-stream")
